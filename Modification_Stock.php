...
<head>
  ...
  <title>Chaustore - Back-office - Stock Modification</title>
  ...
  if (!empty($_POST)) {
    ...
    if (empty($error)) {
      ...
      if (is_null($result)) {
        $countResult=0;
        echo "<br>";
        $ok="Le stock du produit N° ".$prod_id." à modifier avec la pointure ".$taille_id." n'existe pas ...<BR/>Veuillez vérifier les données.";
        echo $ok;
      }
      else {
        $countResult=1;
        ...
        if ($req) {
          $ok="Vos modifications ont été envoyées : Le stock du produit N° ".$prod_id." avec la pointure ".$taille_id." à modifier a été mise à jour. La nouvelle quantité est égale à : ".$stockNew;
        }
        else {
      ...
      $ok = "<br/>Les données du 'Stock' pour le produit n° ".$prod_id." sont à jour.";
